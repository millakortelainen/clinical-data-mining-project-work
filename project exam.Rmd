...
b)  Make a box plot of the CA-125 values. What is dynamic range and median of the CA-125 values?
...
```
c)  How many patients are eligible to the analysis approach you have chosen?
...
```{r}
#minDate, maxDate, minTreatment, lambda
...
summary(model)
```
# Predictin overall survival with CA-125 values
...
model
ggsurvplot(model) + labs(title = "Kaplan-Meier survival analysis of overall survival.")
...
d) What clinical outcome(s) (as column names in the file clinical_variables.xlsx) would you use  in the analysis of i) chemotherapy response and ii) overall survival?

I would use ``pltainium_free_interval` to estimate chemotherapy response and `days_to_death` to estimate overall survival.
